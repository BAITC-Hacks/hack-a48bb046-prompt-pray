...
Форма `/tasks/new` передаёт `locale: ru|kk|en` при создании черновика; по умолчанию
`ru`. Язык вопросов можно выбрать перед сохранением. Повторное открытие загружает
...
T-19 ещё отсутствует в этой ветке: общий переключатель языка интерфейса здесь
не реализован. При интеграции задайте начальный язык нового черновика из locale
T-19, сохранив явный выбор в форме. Не связывайте существующий `draft.locale`
двусторонне с языком UI, не задавайте `NuxtPage` ключ по языку и не перезагружайте
вопросы при переключении UI — это может стереть ввод. Этот сценарий нужно проверить
после объединения T-19 с T-17.
