...
- **Прикладные ошибки** имеют формат `{"detail": "...", "code": "not_found"}`.
  Ошибки валидации запросов возвращают стандартный HTTP 422 FastAPI с массивом `detail`, без `code`.
- **БД.** У каждого сервиса своя база и свой пользователь Postgres, чужие базы ему недоступны.
  Каталог при старте применяет Alembic-миграции до `head`, включая язык черновика и версию карточки.
  Процедура обновления и резервного копирования — в [README каталога](catalog_service/README.md#миграции-схемы-t-15).
  Auth и example создают таблицы через `create_all`; auth дополнительно обновляет схему роли пользователя.
...
При `start.bat` / `./start.sh` с `ENV=development` после запуска сервисов создаются
демоаккаунты: `user1` (бизнес), `user2` и `user3` (студенты), пароль у всех —
`password123`. На странице входа можно указать имя пользователя или email
(`user1@demo.example.com` и аналогично). API входа сохраняет поле `email`,
но теперь принимает в нём также имя пользователя; email приводится к нижнему регистру,
имя пользователя чувствительно к регистру.

Создаются три опубликованные учебные задачи разной полноты, один черновик и два
отклика на первую задачу. Выбор команды остаётся за бизнесом. Демо не требует AI-ключа.
Повторный запуск не дублирует задачи и отклики и не заменяет отредактированные карточки.
Данные хранятся в локальной БД; в production демо автоматически не создаётся.
`--no-demo` отключает заполнение. `--reset-db` удаляет **все** локальные SQLite-данные,
после чего при обычном запуске демоданные создаются заново.
...
Все пути ниже используют префикс `/api/v1/catalog`:

- Бизнес создаёт `/drafts`, запрашивает `/drafts/{id}/questions`, сохраняет ответы
  через PATCH `/questions/{id}` и собирает `/drafts/{id}/card`.
  API разрешает сборку без вопросов и ответов: используются описание и явно переданные поля.
  При вызове генерации AI должен вернуть от 3 до 7 вопросов.
- Карточка редактируется через PATCH `/tasks/{id}`, рейтинг пересчитывается.
  PATCH, подтверждение и публикация требуют `expected_version` из последнего ответа карточки.
  Например, для карточки версии 1: POST `/tasks/{id}/confirm` с
  `{ "confirmed": true, "expected_version": 1 }`, затем POST `/tasks/{id}/publish`
  с `{ "expected_version": 2 }`, если подтверждение вернуло версию 2.
  Каждая успешная операция увеличивает версию; после публикации она находится в `task.version`.
  Устаревшая версия возвращает `409 catalog_version_conflict`; нужно загрузить карточку
  и явно согласовать изменения. Редактирование снимает публикацию до повторных подтверждения и публикации.
- Студент отправляет `/tasks/{id}/proposals` с `team_id`, идеей, планом и необязательной
  ссылкой на прототип. Членство в команде не проверяется; `user_id` берётся из токена.
  Владелец просматривает отклики и создаёт `/tasks/{id}/decisions` с явным
  `selected_proposal_ids` (пустой список означает отказ всем).
...
## Календарь посещений и награды бизнеса

Миграция `0003_rewards` создаёт `daily_visits` и `reward_events` при старте каталога;
`0004_engagement` добавляет кошельки и отдельную историю полезных действий.
Данные существующих задач сохраняются; автоматического начисления за прошлые действия нет.
Новые публикации и решения, в том числе для существующих задач, участвуют в начислении.

Все методы доступны через gateway с префиксом `/api/v1/catalog`:

- `POST /gamification/check-in` — бизнес или студент отмечает текущий день по серверному UTC.
  Повторный запрос за тот же день не создаёт вторую запись. Дату передать нельзя.
- `GET /gamification?month=2026-09` — баланс монеток и накопительная репутация за всё
  время, текущая/лучшая серия, дни выбранного месяца с посещениями и историей наград.
  Серия сохраняется до конца следующего дня после последнего посещения; пропуск
  целого дня её прерывает. Пустые дни и високосные месяцы включены в календарь.
  `current_streak`, `best_streak`, `active_days` относятся только к посещениям.
  `action_current_streak`, `action_best_streak`, `action_days` и `last_action_date`
  сохраняют отдельную статистику действий и не управляют огоньками календаря.
- `GET /gamification/leaderboard` — для авторизованных участников: до 10 бизнесов
  по репутации/монеткам и до 10 команд по числу задач, в которых команда сейчас
  выбрана, затем по числу откликов. Учитывается последнее ручное решение по задаче.
  Отказ всем убирает задачу из прогресса команды. Категорий пока нет, сравнение общее.
  Отображаются технические идентификаторы без email, описаний и контактных данных.
  `team_id` пока задаётся при отклике; подтверждённых профилей/членства команд в MVP нет.

Награды бизнеса: публикация при рейтинге ≥70 — 50 монеток / 10 репутации;
при ≥90 — дополнительно 30 / 10; первый непустой ручной выбор по задаче — 20 / 5.
Кошельки также сохраняют +5 за первое заполнение каждого поля карточки бизнесом,
+10 студенту за первый отклик на задачу и +100 за принятый бизнесом отклик.
Студенческие награды выдаются один раз на пользователя и задачу независимо от team_id.
Каждый вид награды выдаётся один раз на задачу в транзакции исходного действия.
Перепубликация, снижение/восстановление рейтинга и повторный выбор не дают дубликатов.
Репутация накопительная: отмена выбора не отзывает ранее выданную награду бизнеса.
Посещения не дают монеток. Монетки и репутация не изменяют рейтинг каталога и не
назначают команды. Защита от создания множества отдельных задач вне объёма этой механики.
