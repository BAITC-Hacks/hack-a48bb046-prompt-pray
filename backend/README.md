...
демоаккаунты: `user1` (бизнес), `user2`–`user6` (пять студенческих команд), пароль у всех —
...
В новой базе создаются пять опубликованных карточек с рейтингами 20/40/60/80/100,
пять отдельных черновиков (плюс пять исходных черновиков карточек), пять откликов
с профилями команд, сроками и ссылками на статические учебные макеты.
Выбор команды остаётся за бизнесом. Демо не требует AI-ключа.
Повторный запуск не дублирует данные и не заменяет пользовательские правки.
Реестр ID хранится в `data/demo-seed.json`; сохраняйте его вместе с локальной базой.
Старые демозаписи сохраняются, поэтому на существующей базе откликов может быть больше пяти.
Для уже запущенного backend: `python scripts/seed_demo.py`.
Состав набора и ограничения профилей команд: [демоданные](../docs/demo-data.md).
